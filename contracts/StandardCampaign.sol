...
contract StandardCampaign is Owned, Campaign {
  ...
  function stage() public constant returns (uint256) {
    ...
    // if n >= rE and aR < fG 
    // then the crowdfund is a failure and the refund period has expired
    } else if(block.number >= refundExpiry 
      && earlySuccess == false
      && amountRaised < fundingGoal) {
      return uint256(Stages.CrowdfundRefundExpiry);
  ...
  // after the refund period expiry,
  // return the balance held in contract to the beneficiary
  function refundToBeneficiary() 
    public 
    atStage(Stages.CrowdfundRefundExpiry)
    onlybeneficiary() {
  ...
  function StandardCampaign(string _name,
    ...
    // the refund expiry in blocks
    refundExpiry = _refundExpiry;
  ...
  // the current refund time expiry (future block number)
  uint256 public refundExpiry;
